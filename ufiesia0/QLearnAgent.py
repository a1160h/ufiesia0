...
class BaseAgent:
    ...
    def learn(self, obs, act, rwd, done, next_obs, is_learn=False):
        ...
        delta = self.get_delta(obs, act, rwd, done, next_obs) # 優先度付き経験再生用
        
        if hasattr(self, 'replay_memory'): # 経験再生メモリがある場合
            self.replay_memory.add(obs, act, rwd, done, next_obs, delta)
            obs, act, rwd, done, next_obs, _ = self.replay_memory.sample()
    ...
    def get_delta(self, obs, act, rwd, done, next_obs):
        """ ターゲットと現状の差を求める """
        #print(obs, act, rwd, done, next_obs)
        target  = self._get_target(rwd, done, next_obs)
        current = self.get_Q(obs)[act]
        delta = target - current
        #print('target =', target, 'current =', current, 'delta =', delta)
        return delta
...
class NetQAgt(BaseAgent):
    ...
    def _build_model(self, **kwargs):
        ...
        self.n_dense = kwargs.pop('n_dence',          32)
        ml_act       = kwargs.pop('ml_act',       'Tanh')
        optimize     = kwargs.pop('optimize', 'Momentum')
        model = NN.NN_m(
            self.n_act,                  # 出力数
            ml_nn  = self.n_dense,       # 中間層のニューロン数
            ml_act = ml_act,             # 中間層の活性化関数
    ...
    def _model_update(self, obs, act, done, target):
        ...
        self.model.backward()
        self.model.update(eta=self.alpha, g_clip=10) # g_clip追加20230202          
...
class ReplayMemory:  
    """ 経験を記録するクラス 項目ごとに記録を蓄積 """
    def __init__(self, **kwargs):
        self.memory_size = kwargs.pop('replay_memory_size', 1000)# 100)
        self.batch_size  = kwargs.pop('replay_batch_size',   100)# 20)
    ...
    def sample(self): 
        """ batch_size分をランダムにサンプルし、項ごとに並べてarrayで返す """
        if len(self.memory) < self.batch_size: # batch_sizeに満たない時はそのまま
            data = self.memory
        else:                                  # batch_sizeを超えたらサンプル
            data = random.sample(self.memory, self.batch_size)    
        data =[np.array([x[i] for x in data]) for i in range(self.items)]
        #print(len(data[0]))
        return data
...
class ReplayQAgt(NetQAgt):
    ...
    def _get_target(self, rwds, dones, next_obss):
        """ ターゲット作成:バッチ処理 ~で真偽反転 """
        batch_size = 1 if type(dones) is bool else len(dones)
        next_obss = next_obss.reshape(batch_size, -1)
